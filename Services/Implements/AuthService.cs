...
namespace QLKhachSanAPI.Services.Implements
{
    ...
    public class AuthService : IAuthService
    {
        ...
        public async Task<AuthResult> LoginEFAsync(LoginModel model, string? userAgentString)
        {
            ...
            var accessToken = await _authTokenService.GenerateAccessTokenAsync(user, roles);
            ...
            var accesstokenValidTo = model.RememberMe ? DateTime.UtcNow.AddYears(1) : accessToken.ValidTo;

            // Set the expiration time for the access token based on "Remember Me" option
            var accesstokenExpiration = model.RememberMe ? DateTime.UtcNow.AddYears(1) : DateTime.UtcNow.AddHours(1);
        ...
        public async Task<AuthResult> Refresh(RefreshTokenRequest model)
        {
            ...
            var newAccessToken = await _authTokenService.GenerateAccessTokenAsync(user, roles);
